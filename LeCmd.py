...
# 校准值
Calibration = (500, 500, 303, 500, 500, 725, 500, 500, 500, 500, 500, 697, 500, 500, 275, 500, 500, 500)
...
def cmd_i001(par):  # 指令001 舵机运动
    ...
    for i in range(0, len(Servos), 2):
        if Servos[i] > RobotJoint or Servos[i] < 1 or Servos[i+1] > 1000 or Servos[i+1] < 0:
...
#    print(par)
    if not par is None:
        try:
            threading.Thread(target=Serial_Servo_Running.run_ActionGroup, args=(data[0], par)).start()
...
def read_deviation():   # 读取舵机内部偏差
    d = []
    for i in range(1, RobotJoint + 1, 1):
...
def cmd_i008(sock, data=[]):      # 修改偏差
    ...
    ########################################################
    if not len(Calibration) == RobotJoint and not len(upper_d) == RobotJoint:
        print("偏差数量错误")
        s = "I008-dev-no\r\n"
        sock.sendall(s.encode())
        sys.exit()
